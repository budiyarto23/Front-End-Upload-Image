...
class ManageBrand extends Component {
    ...
    componentDidMount() {
        axios.get('https://multifilt-image.herokuapp.com/brand/getlistbrand')
        .then((res) => {
    ...
    onBtnAddClick = () => {
        if(document.getElementById("AddBrandImage").files[0] !== undefined) {
            ...
            axios.post("https://multifilt-image.herokuapp.com/brand/addbrand", formData, headers)
            .then((res) => {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Are you sure to delete?')) {
            axios.delete('https://multifilt-image.herokuapp.com/brand/deletebrand/' + id)
            .then((res) => {
    ...
    onBtnUpdateClick = (id) => {
        ...
        axios.put("https://multifilt-image.herokuapp.com/brand/editbrand/" + id, formData, headers)
        .then((res) => {
        ...
        var listJSX = this.state.brandList.map((item) => {
            ...
            return (
                <tr>
                    <td>{item.id}</td>
                    <td>{item.nama}</td>
                    <td><img src={`https://multifilt-image.herokuapp.com${item.image}`} alt={item.nama} width={100} /></td>
